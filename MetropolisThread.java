...
public class MetropolisThread extends Thread{
	...
	// calls the MetropolisAlgorithm() N_m times, and computes mean magnetization (eq. 1.7.1a) and mean pair correlation (eq. 1.7.2a) for this thread.
	public void run(){		
		double m_sum = 0;
		double cp_sum = 0;
		for(int i = 0; i < N_m; i++){
	...
	//	returns the mean magnetization.
	public double getMagMean(){
		return m_mean;
	}
	
	//	returns the mean pair correlation.
	public double getPairCorrMean(){
		return cp_mean;
	}
	
	/*	computes magnetization (eq. 1.3.1) and pair correlation (eq. 1.3.2) of the optimal configuration which is the result of the i^th call to MetropolisAlgorithm(). 
	...
	*/
	...
	//	selects a pseudorandom number, rand_index where, 0 <= rand_index < n.
